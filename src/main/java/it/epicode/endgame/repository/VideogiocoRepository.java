...
public interface VideogiocoRepository extends JpaRepository<Videogioco, Integer>, PagingAndSortingRepository<Videogioco, Integer> {
    ...
    @Query("SELECT v FROM Videogioco v WHERE LOWER(v.titolo) LIKE %:titolo% ORDER BY v.titolo ASC")
    ...
    @Query("SELECT v FROM Videogioco v WHERE lower(v.genere) LIKE lower(concat('%', :genere, '%'))")
